...
namespace serial {
	...
	public:
		...
		std::future<std::vector<uint8_t>> receiveAsync(size_t const num_bytes, unsigned int timeout, std::string_view trigger="");

		void transmit(std::vector<uint8_t> const& data);
		void transmit(std::string_view const& data);
	...
	private:
		...
		std::vector<uint8_t> readBufferTimeout(size_t len, std::string_view trigger);
	...
	std::vector<uint8_t> Serial::readBufferTimeout(size_t len, std::string_view trigger)
	{
		...
		while (res.size() < len && !timeout)
		{
			...
			else
			{
				...
				if (b > -1)
				{
					...
					if(res.size()<trigger.size()) continue;
					auto compare = reinterpret_cast<const char * const >(res.data()) + res.size() - trigger.size();
					if(trigger == compare){
						//printf("WHOHOHO, %s)", compare);
						return res;
					};
				}
			}
	...
	std::future<std::vector<uint8_t>> Serial::receiveAsync(size_t const num_bytes, unsigned int timeout, std::string_view trigger)
	{
		timeoutVal = timeout;
		return std::async(std::launch::async, boost::bind(&Serial::readBufferTimeout, this, num_bytes, trigger));
	}
	...
	void Serial::transmit(std::string_view const& data)
	{
		boost::asio::write(serial_port, boost::asio::buffer(data));
	}
	...
	//Waits for single key from terminal. Supports Viditel control codes.
	std::vector<uint8_t> Serial::getKey(){
		std::vector<uint8_t> val = {0,0,0,0,0};
		while(true){
            auto future = receiveAsync(1);
